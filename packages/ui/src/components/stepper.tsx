...
function StepperSeparator({ className }: React.ComponentProps<"div">) {
  ...
  return (
    <div
      ...
      data-state={state}
      className={cn(
        "bg-muted rounded-sm m-0.5",
        // `flex-1` fills a width-defined container; `min-w-8` keeps the line visible even inside a
        // content-sized (auto) flex parent, where flex-1 would otherwise collapse to zero.
        orientation === "horizontal" ? "h-0.5 min-w-8 flex-1" : "h-12 w-0.5",
        className,
      )}
    />
  );
...
// displayName is load-bearing: Stepper counts children whose type.displayName === "StepperItem".
