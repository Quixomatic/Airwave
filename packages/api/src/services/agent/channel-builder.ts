...
- On \`title\`, \`contains\` is a SUBSTRING match (\`title contains "Bear"\` matches anything with "Bear") and \`equals\` is EXACT. Prefer \`contains\` for shows/franchises; use \`equals\` only for one precise title — and note Plex titles often carry the year (a show may be stored as "Bluey (2018)", not "Bluey"), so exact needs the full stored title.
