...
export function qualityParams(id) {
 ...
 */
export function clientProfileExtra(caps, protocol = "http", httpContainer = "mkv") {
 ...
 */
export function progressiveContainer(caps) {
    const c = caps.directContainers;
    if (c.includes("mkv"))
        return "mkv";
    if (c.includes("mpegts") || c.includes("ts"))
        return "mpegts";
    return null;
}
