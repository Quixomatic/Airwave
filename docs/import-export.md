# Import / Export (move a lineup between instances)
...
Note: the durable-execution mechanics — `"use workflow"` vs `"use step"`, checkpointing, retries, why editing a workflow requires re-running `bunx workflow build` and a server restart — are shared with the AI lineup builder and documented in **[`docs/workflows.md`](./workflows.md)** rather than repeated here. (The separate in-process job scheduler is covered in [`docs/jobs.md`](./jobs.md).)
