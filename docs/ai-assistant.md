...
Both share the same toolbox in `tools.ts` and both stamp their output `aiGenerated` — so `clear_ai_generated` cleanly reverses whatever *either* one made. The durable builder is documented in full in **[`docs/workflows.md`](./workflows.md)**. If `WORKFLOW_ENABLED` is unset, the lineup builder is simply unavailable — but the chat assistant still works, because it never touched the workflow engine.
