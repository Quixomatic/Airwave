...
function PackagesList() {
  ...
  const refreshStyling = async () => {
    const ok = await confirm({
      title: "Refresh package styling?",
      description: (
        <>
          Re-applies the built-in preset catalog's styling (name, description, icon, tint, and sort order)
          to your auto-generated packages. It doesn't add, remove, or change any channels.
        </>
      ),
      confirmLabel: "Refresh styling",
    });
    if (!ok) return;
    setRefreshing(true);
    try {
