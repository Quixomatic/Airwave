...
1. **Open a TV app.** Pick your platform from the [availability table](https://github.com/Quixomatic/Airwave#platform-availability) in the README — Apple TV, iPad, Android TV, Fire TV, and LG webOS have native apps; **any browser** works via the `tvweb` Docker role (an auth-gated web player). The admin's built-in browser player at `/watch/:channel` is handy for a quick check without leaving the panel.
