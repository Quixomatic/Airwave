...
> **Store status.** Airwave is live on the major TV stores: **Apple TV** (App Store), **Android TV / Google
> TV** (Google Play), **Fire TV** (Amazon Appstore), and **Roku** (Roku Channel Store), with **iPad** in App
> Store review. **LG webOS** runs daily on a real LG C2 and installs by sideload while its store listing is in
> progress. The **desktop app** ships on **Windows and macOS** (Apple Silicon + Intel) as signed,
> self-updating installers; **Linux** shares the same Tauri + mpv core and is next. Because Airwave is
> source-available, you can also **build and sideload** any native app yourself.
