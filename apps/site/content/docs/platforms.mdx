...
> on a real Roku Streaming Stick 4K, and the **desktop app** ships on **Windows and macOS** (Apple Silicon
> + Intel) as signed, self-updating installers — validated on real hardware; Linux shares the same Tauri +
> mpv core and is next. Publishing them to the App Store / Google Play /
...
either way. **Linux** shares the same core and is the next build target. Distributed as a small signed
installer per OS, or built from source.
