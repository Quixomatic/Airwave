...
> **Running a local model?** Beyond tool-calling, the other thing that matters is **turning off reasoning /
> "thinking" mode** — with it on, the planner call tends to time out at zero output tokens and look
> incapable when it isn't. See [Local & self-hosted models](/docs/ai-assistant/local-models) for the full
> guide, the planner-timeout notes, and a known-good local config.
