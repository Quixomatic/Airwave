...
install (override with `--dir` / `-Dir`). Secrets (Postgres password, auth secret, a stable Plex client id) are
generated for you, and the AI lineup engine is on by default (add an AI provider key in the admin to use it).
...
- **Advanced setup:** add `--advanced` (`-Advanced`) to bind host paths for the database and bumper music,
  serve the browser TV player, or add extra CORS origins. The default keeps it simple (Docker named volumes).
