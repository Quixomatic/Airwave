...
<Alert type="info" title="Just want it running?">
There's a [one-line installer](/docs/self-hosting#one-command-fastest) that does everything on this page
for you. Setting it up by hand (below) is worth it if you want to understand your deployment, tweak volumes
and ports, or drop it into Dockge / Portainer.
</Alert>
...
profile (see [Roles](/docs/self-hosting/roles)).
...
<Alert type="warn" title="Get the public URLs right">
...
first-deploy snag (see [Gotchas](#gotchas-that-actually-bite)).
</Alert>
...
<Alert type="info" title="The admin seed is optional but recommended">
