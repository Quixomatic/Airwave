 ...
 * run. The choice persists in localStorage. In production `dev` is false, so it always renders v2 (the chosen
 * hero) with no visible chrome — the switcher is dev-only scaffolding, and V1 is kept for switching back.
 */
export function HeroToggle({
  ...
  useEffect(() => {
    ...
    if (saved === "v1" || saved === "v2") setVariant(saved);
  }, [dev]);
  ...
  // Production always shows V2 (the chosen hero). In dev, the toggle picks and persists the variant — V1 is
  // kept around so it can be switched back to.
  const active = dev ? variant : "v2";
