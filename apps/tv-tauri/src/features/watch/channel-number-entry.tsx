 ...
 * (all live on the "/" route), typing a digit arms a channel-number buffer shown in a top-center
 * slide-in; **OK / Enter** (and only OK — a toddler mashing digits must never tune on its own)
 * commits it, tuning the channel full-screen if it exists, or flashing red briefly if it doesn't.
 * An arrow key breaks out and passes through to normal navigation; Back breaks out and is consumed;
 * and a stretch of inactivity quietly dismisses the buffer WITHOUT tuning (never a commit).
 *
 * CH▲/▼ (desktop: `]`/`[`, or a remote's channel keys) steps the lineup by one via the provider's
 * `channelStep` (clamped, in-flight-locked), abandoning any in-progress number entry first.
 *
 * Rendered once by PlayerProvider (always mounted, above the router) so it's armed globally. It
 * registers on the OVERLAY key layer and only consumes what it uses — digits/CH while armed, and
 * OK/Back/arrows while a buffer is active — so the guide's and player's own layers are untouched
 * otherwise. Guards off text inputs and open dropdown menus.
 *
 * Faithful port of tv-web `features/watch/channel-number-entry.tsx`. tv-tauri seam: the pill drops
 * below the custom window titlebar (`--titlebar-h`) so it never sits over the min/max/close controls.
 */
...
export function ChannelNumberEntry() {
  ...
  // Channel surf sits ABOVE this (MODAL) and swallows everything.
  useKeyLayer({
    id: "number-entry",
    ...
    <AnimatePresence>
      {(buffer.length > 0 || flash) && (
        <motion.div
          ...
          style={{
            // Top-CENTER, dropped below the custom window titlebar (like the full-chrome channel pill
            // and Back button) so it clears the min/max/close controls. `x: -50%` stays constant while
            // `y` animates the slide.
            position: "fixed",
            ...
            padding: "16px 34px",
            borderRadius: 18,
            // Same glass treatment as the full-chrome channel pill. Border goes red on an invalid number.
